...
//得到每个段子路径的正则表达式  `<h4> <a href="(?:s(.*?))``<h4> <a href="(.*?)"`
...
func (this *Spider) DoWork() {
	...
	for {
		...
		content, rcode := HttpGet(url1)
		if rcode != 200 {
			fmt.Println("GETHTTP err rcode = ", rcode)
			continue;
		}
		...
		result := myreg.FindAllStringSubmatch(url, -1)
...
func duanzi_spider(url string) {
	
	url := "https://www.neihanba.com"
}
func main() {
	sp := new(Spider)
	sp.DoWork()
